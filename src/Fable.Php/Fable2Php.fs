...
module PhpUnion =
    let fSharpUnion = { Namespace = None; Name = "FSharpUnion"; Fields = []; Constructor = None; Methods = []; Abstract = true; BaseType = None; Interfaces = []; File = "fable-library/FSharp.Core.php"; OriginalFullName = "FSharp.Core.FSharpUnion"}

module Core =
    let icomparable = { Namespace = None; Name = "IComparable"; Fields = []; Constructor = None; Methods = []; Abstract = true; BaseType = None; Interfaces = [] ; File = "fable-library/FSharp.Core.php"; OriginalFullName = "System.Collections.IComparable"}
...
let unscopedIdent name =
    { Name = name; Namespace = None; Class = None }
        ...
        { Namespace = Some com.PhpNamespace
          ...
          Constructor =
            Some {
                ...
                Body = [ for e in case.UnionCaseFields ->
                            PhpAssign(PhpField(PhpVar("this", None), StrField e.Name, None) , PhpVar (e.Name, None) )
                ]
            }
          ...
          Methods = [
              { PhpFun.Name = "get_FSharpCase"
                ...
                PhpFun.Static = false
                PhpFun.Body =
                  [ PhpStatement.PhpReturn(PhpConst(PhpConstString(case.Name)))] }
              { PhpFun.Name = "get_Tag"
                PhpFun.Args = []
                PhpFun.Matchings = []
                PhpFun.Static = false
              ...
              { PhpFun.Name = "CompareTo"
                ...
                PhpFun.Body =
                                  [ for e in case.UnionCaseFields do
                                        ...
                                        | Fable.Type.Number _ ->
                                            PhpAssign(cmp,
                                                PhpTernary( PhpBinaryOp(">",
                                                                PhpField(PhpVar("this",None), Prop.Field { Name = e.Name; Type = convertType com e.FieldType }, None),
                                                                PhpField(PhpVar("other", None), Prop.Field { Name = e.Name; Type = convertType com e.FieldType }, None) ),
                                        ...
                                        | _ ->
                                            PhpAssign(cmp,
                                                PhpMethodCall(PhpField(PhpVar("this",None), Prop.Field { Name = e.Name; Type = convertType com e.FieldType }, None),
                ...
                { Namespace = Some com.PhpNamespace
                  ...
                  Methods = [ { PhpFun.Name = "get_FSharpCase";
                              ...
                              { PhpFun.Name = "get_Tag"
                                ...
                                PhpFun.Static = false
                                PhpFun.Body =
                                    [ PhpStatement.PhpReturn(PhpConst(PhpConstNumber (float i)))] }
                              { PhpFun.Name = "CompareTo"
                                PhpFun.Args = ["other"]
                                PhpFun.Matchings = []
                                PhpFun.Static = false
                                PhpFun.Body =
                                                  [ let cmp = PhpVar(com.MakeUniqueVar "cmp",None)
                                                    PhpAssign(cmp,
                                                        PhpTernary( PhpBinaryOp(">",
                                                                        PhpMethodCall(PhpVar("this",None), PhpIdent(unscopedIdent "get_Tag"), []),
                                                                        PhpMethodCall(PhpVar("other", None), PhpIdent(unscopedIdent "get_Tag"), []) ),
                                                                        PhpConst(PhpConstNumber 1.),
                                                                           PhpTernary(
                                                                               PhpBinaryOp("<",
                                                                                   PhpMethodCall(PhpVar("this",None), PhpIdent(unscopedIdent "get_Tag"), []),
        ...
        { Namespace = Some com.PhpNamespace
          ...
          Constructor =
            Some {
                ...
                Body = [ for e in info.FSharpFields ->
                            PhpAssign(PhpField(PhpVar("this", None), StrField e.Name, None) , PhpVar (e.Name, None) )
                ]
            }
          Methods = [
...
and getPhpTypeForEntity (com: IPhpCompiler) (entity: Fable.Entity) =
    ...
    | Some path ->
        ...
        | :? Fable.Transforms.FSharp2Fable.FsEnt as fs ->
            ...
            { Name = fixName fs.FSharpEntity.CompiledName; Namespace = ns; Class = None }

        | _ ->
            let rootModule = com.GetRootModule(path)
            { Name = fixName entity.DisplayName; Namespace = Some (nsreplacement rootModule); Class = None}
    | None ->
        { Name = fixName entity.DisplayName; Namespace = Some ""; Class = None }


let withNamespace ns name =
   { Name = name; Namespace = ns; Class = None }
...
let convertTest (com: IPhpCompiler)  test phpExpr =
    let phpIsNull phpExpr = PhpFunctionCall(PhpIdent (unscopedIdent "is_null"), [phpExpr])

    match test with
    | Fable.TestKind.UnionCaseTest(tag) ->
        // union test case is implemented with a ->get_Tag() test
        PhpBinaryOp("==",PhpMethodCall(phpExpr, PhpIdent (unscopedIdent "get_Tag"), []), PhpConst(PhpConstNumber(float tag)))
                ...
                | None ->
                    ...
                    | Some prop ->
                        ...
                        | Fable.IdentExpr id ->
                            ...
                            if prop.Static then
                                PhpFunctionCall(PhpIdent { Namespace = None; Class = Some (fixName id.Name); Name = name}, [])
                            else
    ...
    | Fable.Import(info,t,_) ->
        ...
        | "" ->
            ...
            | Some true ->
                ...
                PhpGlobal(name)
            | _ ->
                PhpIdent (unscopedIdent (fixName info.Selector))
        ...
        | ns ->
            ...
            | _ ->
                ...
                if sepPos >= 0 then
                    ...
                    PhpIdent { Namespace = Some ns; Class = None; Name = fixName info.Selector }
...
and convertMatching (com: IPhpCompiler) input guard thenExpr elseExpr expr returnStrategy =
    if (canBeCompiledAsSwitch expr input) then
        ...
        [ PhpSwitch(PhpMethodCall(inputExpr, PhpIdent( unscopedIdent "get_Tag"), []),
